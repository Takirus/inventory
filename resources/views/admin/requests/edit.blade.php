    ...
    <div class="row justify-content-center">
      <div class="col-md-6">
        ...
        <form action="{{ route('admin.requests.update', $request->id) }}" method="POST">
          ...
          <div class="mb-3">
            ...
            <select class="form-select" id="equipment_id" name="equipment_id">
                ...
                @foreach($equipments_user as $equipment_user)
                    <option {{ $request->equipment_id  == $equipment_user->id ? 'selected' : ' ' }}
          ...
          <div class="mb-3">
            <label for="status_id" class="form-label">Выберите статус для заявки:</label>
            <select class="form-select" id="status_id" name="status_id">
                <option value="" selected disabled>Статус заявки</option>
                @foreach($all_statuses as $status)
                    <option {{ $request->current_status_id  == $status->id ? 'selected' : ' ' }}
